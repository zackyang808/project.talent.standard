...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        {
            string TalentPhotoUrl = await _profileService.UpdateTalentPhoto(_userAppContext.CurrentUserId, Request.Form.Files[0]);
            if (!string.IsNullOrEmpty(TalentPhotoUrl))
            {
                return Json(new { Success = true, TalentPhotoUrl = TalentPhotoUrl });
            }
